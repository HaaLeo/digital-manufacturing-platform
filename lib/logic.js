 ...
 * @param {org.usecase.printer.ConfirmPrintingJob} uploadInfo
 * @transaction
 */
function ConfirmPrintingJob(uploadInfo) {
...
function requestBlueprint(buyingInfo) {
    ...
    printingJob.qualityRequirement = buyingInfo.qualityRequirement;
    printingJob.txID = '';
...
/**
 * Add Evaluate Report
 * @param {org.usecase.printer.EvaluateReport} jobInfo - evaluating a new report
 * @transaction
 */

function EvaluateReport(jobInfo){
    debugger;
    var qualityRequirement = jobInfo.printingJob.qualityRequirement.txID;
    var qualityReport = jobInfo.qualityReport;

    let qualityRequirementPressure = jobInfo.peakPressure;
    let qualityRequirementTemperature = jobInfo.peakTemperature;

    let qualityReportPressure = jobInfo.pressure;
    let qualityReportTemp = jobInfo.temperature;

    var requirementsMet = false;
    debugger;
    if (jobInfo.peakPressure>jobInfo.pressure && jobInfo.peakTemperature>jobInfo.temperature){
        debugger;
        requirementsMet = true;
    }

    var evaluationResult = getFactory().newResource(NS, 'EvaluationResult',  Math.random().toString(36).substring(3));
    evaluationResult.txID = '';
    evaluationResult.requirementsMet = requirementsMet;
    evaluationResult.printingJob = jobInfo.printingJob;
    evaluationResult.customer = jobInfo.customer;
    evaluationResult.qualityReport = jobInfo.qualityReport;
    debugger;
    return getAssetRegistry(NS + '.EvaluationResult')
        .then(function (assetRegistry) {
            return assetRegistry.add(evaluationResult);
        });
